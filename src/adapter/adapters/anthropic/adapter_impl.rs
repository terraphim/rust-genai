...
mod tests {
	...
	// region:    --- Endpoint URL normalization tests
	//
	// Regression tests for the bug where `format!("{base_url}messages")` produced
	// malformed URLs (e.g. `https://api.minimax.io/anthropicmessages`) for
	// Anthropic-compat gateways whose base URL didn't end in `/v1/`. The fix
	// added a three-way shape detection in `get_service_url`.
	...
	// endregion: --- Endpoint URL normalization tests
}
